...
GLuint filter;                              /* which of the 3 filters below to use */
GLuint fogmode[]={GL_EXP,GL_EXP2,GL_LINEAR};
GLuint fogfilter;
GLfloat fogcolour[]={0.5,0.5,0.5,1};
...
int initgl() {
  ...
  glClearColor(0.5,0.5,0.5,0);              /* fog colour! */
  glClearDepth(1);                          /* depth buffer setup */
  glEnable(GL_DEPTH_TEST);                  /* enable depth testing */
  glDepthFunc(GL_LEQUAL)                    /* type of depth testing */;
  glHint(GL_PERSPECTIVE_CORRECTION_HINT,GL_NICEST); /* really nice perspective calculations */
  /* unfortunately, need to do this for each resize */
  if(!loadgltexture("16crate.bmp",&texture[0])) return 0;
  ...
  glFogfv(GL_FOG_COLOR,fogcolour);          /* set fog colour from the array */
  glFogf(GL_FOG_DENSITY,0.35);
  glHint(GL_FOG_HINT,GL_DONT_CARE);         /* fog hint value, whatever this is */
  glFogf(GL_FOG_START,1);                   /* start depth of fog */
  glFogf(GL_FOG_END,5);                     /* end depth of fog */
  glEnable(GL_FOG);
  return 1;
}
