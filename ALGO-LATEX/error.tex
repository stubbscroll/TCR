...
\begin{itemize}
	\item Don't use DP if the recurrence has cycles! Consider doing a simultaneous DP/gaussian elimination where the gaussian elimination operates on each strongly connected component, working its way through the topologically sorted component graph.
\end{itemize}
...
\begin{itemize}
	\item Don't calculate grundy numbers if just \texttt{xor}-ing nim values is enough (and the nim-values are easily obtainable without doing the mex calculations).
\end{itemize}
