...
__kernel void pe184(__global int *a,int x3,int y3) {
	int ix=get_global_id(0),iy=get_global_id(1);
	int x1=ix-N+1,x2=iy-N+1,y1,y2,r=0;
	int idx=ix+iy*N2;
	int a0,a1,a2,a3;
	if(x3 || y3) for(y1=-N;y1<N;y1++) if(x1*x1+y1*y1<N*N && (x1 || y1)) {
		...
		for(y2=-N;y2<N;y2++) if(x2*x2+y2*y2<N*N && (x2 || y2)) {
			...
			a1=x2*y3-x3*y2; if(!a1) continue; if(a1<0) a1=-a1;
			a2=x3*y1-x1*y3; if(!a2) continue; if(a2<0) a2=-a2;
			a3=x1*y2-x2*y1; if(!a3) continue; if(a3<0) a3=-a3;
			if(a0==a1+a2+a3) r++;
		}
	}
	a[idx]=r;
